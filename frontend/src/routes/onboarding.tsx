...
const onboardingTips: AuthTip[] = [
  ...
  {
    icon: <MousePointerClick className="size-4 text-lime" />,
...
function Onboarding() {
  ...
  async function startPccpSetup() {
    ...
    try {
      ...
      setOrder(res.order);
      setRepetition(1);
      setPccpCapturing(true);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not start PCCP setup. Try again.");
    } finally {
      setBusy(false);
    }
  }

  async function handlePccpComplete(clicks: PccpClickWithTiming[]) {
    if (!regToken) return;
    setBusy(true);
    setError(null);
    try {
      ...
      setError(err instanceof Error ? err.message : "PCCP enrollment failed.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <>
      ...
      {pccpCapturing && orderedImages.length > 0 ? (
        <PccpChallenge
          key={`${repetition}-${order.join(",")}`}
          images={orderedImages}
          showViewport
          busy={busy}
          onComplete={handlePccpComplete}
          onCancel={() => {
            setPccpCapturing(false);
            setRegToken(null);
            setImages([]);

 ...
 */
/** Same k-anonymity lookup for a plaintext password. Returns breach count or null on failure. */
export async function checkPasswordBreach(password: string): Promise<number | null> {
  try {
    ...
    for (const line of data.split("\n")) {
      ...
      if (suf?.trim() === suffix) {
        return parseInt(cnt ?? "0", 10) || 0;
      }
    }
    return 0;
  } catch {
    logger.warn("HIBP password lookup failed");
    return null;
  }
}

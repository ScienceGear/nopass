...
/** Send an SMS via the TextBee gateway; logs to the console in dev. */
async function sendSms(phone: string, message: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logger.warn(`[sms] send failed (${res.status}): ${body}`);
    } else {
      logger.info(`[sms] sent → ${maskPhone(phone)}`);
    }
  } catch (err) {
...
/**
 * Generate, persist (hashed) and send a phone OTP. Returns the plaintext for
 * dev surfacing only, or null when the daily quota is exhausted.
 */
export async function sendPhoneOtp(
  ...
  if (!isProduction) logger.info(`[dev] OTP for ${maskPhone(phone)}: ${code}`);
  return code;
}
